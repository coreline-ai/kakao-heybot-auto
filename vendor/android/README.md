...
> **코어라인 헤이봇 운영 식별자:** Android namespace/applicationId와
> `app_process` 진입점은 `ai.coreline.heybot`이다. PD20 운영은 APK를
> `/data/local/tmp/Iris-glm.apk`에 복사한 뒤
> `app_process / ai.coreline.heybot.Main`으로 실행한다.

패키지명 변경은 app_process용 APK 식별자만 바꾸며, `/data/local/private`의
root 전용 토큰·정책·기억·엔진 모드 파일은 유지한다. 기존 패키지를 정리해야
하는 경우에도 먼저 `pm list packages`로 **Iris 소유 package만** 확인하고,
`com.kakao.talk` 또는 카카오톡 DB에는 `uninstall`·삭제를 실행하지 않는다.

### 자체진단

운영 process를 기동하지 않고 Android 내부 안전 진단만 실행하려면 다음처럼 사용한다.
...
`quick`, `integration`, `device`, `canary`를 지원한다. 기본 진단은 외부 생성과
카카오톡 전송을 수행하지 않으며, `canary`도 명시적 승인 전에는 `SKIP`으로 끝난다.
...
#### HTTP API 엔드포인트 (과거 공개 API 문서)

> **보안 빌드 안내:** PD20 운영 빌드는 HTTP 관리 API를 기본 비활성화한다.
> 명시적으로 활성화한 경우에도 loopback(`127.0.0.1`)에서 전용 admin Bearer
> secret을 가진 요청만 허용한다. `/aot`, `/query`, `/decrypt`는 제거됐으며,
> 현재 운영 계약은 [`GLM 자동응답 운영 설정`](../../../docs/GLM_자동응답_운영설정.md)을 따른다.
