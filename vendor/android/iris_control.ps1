...
$IRIS_START_COMMAND = "adb shell 'su root sh -c `"CLASSPATH=/data/local/tmp/Iris.apk app_process / ai.coreline.heybot.Main`"' 2>&1"
...
function Iris-Start {
    ...
    if ($pid_iris) {
        ...
        $job = Start-Job -ScriptBlock {
            adb shell "su root sh -c 'app_process -cp /data/local/tmp/Iris.apk / ai.coreline.heybot.Main'"
        }
