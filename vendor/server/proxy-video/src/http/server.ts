...
import { VideoJobProcessor } from "../orchestration/processor.js";
import { toPublicVideoJob, type CreateVideoJob } from "../jobs/types.js";
...
function validateCreate(value: unknown, config: VideoProxyConfig): CreateVideoJob {
...
export interface VideoServerContext {
  server: Server;
  store: VideoJobStore;
...
export function createVideoServer(config: VideoProxyConfig): VideoServerContext {
  const managerSecret = readSecret(config.managerSecretFile);
  const store = new VideoJobStore(config.databaseFile);
  const grok = new GrokClient(config);
  const processor = new VideoJobProcessor(store, grok, config);
  processor.start();
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      if (request.method === "GET" && url.pathname === "/health") {
        return json(response, 200, { ok: true, service: "proxy-video" });
      }
      if (request.method === "GET" && url.pathname === "/ready") {
        const dependency = await grok.readiness(AbortSignal.timeout(2_000));
        return json(response, dependency.ready ? 200 : 503, {
          ready: dependency.ready,
          dependency: { grok: dependency },
      ...
      if (request.method === "POST" && url.pathname === "/v1/self-test/generate") {
        ...
        processor.kick();
        return json(response, 202, toPublicVideoJob(job));
      }
      if (request.method === "POST" && url.pathname === "/v1/video/jobs") {
        ...
        if (existing) {
          if (existing.chatId !== input.chatId) {
            return json(response, 404, { error: { code: "VIDEO_JOB_NOT_FOUND" } });
          }
          return json(response, 200, toPublicVideoJob(existing));
        }
        if (store.countPending() >= config.queueMaxPending) {
          return json(response, 429, {
            error: { code: "VIDEO_QUEUE_FULL", retryAfterMs: 5_000 },
          });
        }
      ...
      const fileMatch = url.pathname.match(/^\/v1\/video\/jobs\/([0-9a-f-]+)\/file$/);
      if (request.method === "GET" && fileMatch) {
        const job = scopedJob(store, fileMatch[1]!, url);
        if (job?.status !== "succeeded" || !job.artifactPath) {
          return json(response, 404, { error: { code: "VIDEO_FILE_NOT_FOUND" } });
        }
      ...
      const jobMatch = url.pathname.match(/^\/v1\/video\/jobs\/([0-9a-f-]+)$/);
      if (jobMatch) {
        const job = scopedJob(store, jobMatch[1]!, url);
        if (!job) return json(response, 404, { error: { code: "VIDEO_JOB_NOT_FOUND" } });
        if (request.method === "GET") return json(response, 200, toPublicVideoJob(job));
        if (request.method === "DELETE") {
          const cancelled = await processor.cancel(job.id);
          return json(response, cancelled ? 202 : 409, toPublicVideoJob(store.get(job.id)!));
        }
      }
  ...
  return { server, store, processor, grok, shutdown };
}
