...
import type { CreateVideoJob, VideoJob } from "../jobs/types.js";
...
function map(row: VideoRow): VideoJob {
  return {
    ...
    grokJobId: row.grok_job_id ?? undefined,
...
export class VideoJobStore {
  ...
  constructor(path: string) {
    ...
    this.#db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS video_queue ON jobs(status, sequence);
      CREATE INDEX IF NOT EXISTS video_room_queue ON jobs(chat_id, status);
    `);
    this.#db.prepare(
  ...
  findByRequest(requestId: string): VideoJob | undefined {
  ...
  create(input: CreateVideoJob): VideoJob {
    const now = Date.now();
    const id = randomUUID();
    this.#db
      .prepare(
  ...
  setGrokJob(id: string, grokJobId: string): void {
    this.#db
      .prepare("UPDATE jobs SET grok_job_id=?, updated_at=? WHERE id=? AND status='running'")
      .run(grokJobId, Date.now(), id);
  }
  ...
  expiredArtifacts(cutoff: number): VideoJob[] {
    const rows = this.#db
      .prepare(
        `SELECT * FROM jobs WHERE finished_at < ? AND artifact_path IS NOT NULL`,
      )
      .all(cutoff) as unknown as VideoRow[];
    this.#db
      .prepare(
