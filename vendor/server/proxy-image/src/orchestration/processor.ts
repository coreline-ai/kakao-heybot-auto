...
export class ImageJobProcessor {
  ...
  constructor(
    readonly store: ImageJobStore,
    private readonly codex: CodexClient,
    private readonly config: ImageProxyConfig,
  ) {}
  ...
  async cancel(jobId: string): Promise<boolean> {
    ...
    if (job.codexJobId) await this.codex.cancel(job.codexJobId);
    return changed;
  }
  ...
  kick(): void {
    ...
    while (this.#active < this.config.queueConcurrency) {
      ...
      if (Date.now() - job.createdAt > this.config.queueWaitTimeoutMs) {
        this.store.fail(job.id, "IMAGE_QUEUE_WAIT_TIMEOUT");
        continue;
      }
  ...
  private async run(jobId: string, signal: AbortSignal): Promise<void> {
    ...
    const created = await this.codex.create(job.id, job.prompt, signal);
    this.store.setCodexJob(job.id, created.jobId);
    const deadline = Date.now() + this.config.codexJobTimeoutMs;
    let status = created;
    while (status.status === "queued" || status.status === "running") {
      if (Date.now() >= deadline) {
        await this.codex.cancel(created.jobId);
        throw new Error("CODEX_JOB_TIMEOUT");
      }
      await delay(500, signal);
      status = await this.codex.get(created.jobId, signal);
    }
    if (status.status !== "succeeded") {
      throw new Error(status.error?.code || `CODEX_${status.status.toUpperCase()}`);
    }
    const artifact = status.artifacts[0];
    if (!artifact || artifact.mediaType !== "image/png" || status.artifacts.length !== 1) {
      throw new Error("CODEX_ARTIFACT_CONTRACT");
    }
    const data = await this.codex.download(created.jobId, artifact.artifactId, signal);
    const qc = validatePng(data, this.config.imageMaxBytes);
    if (artifact.sha256 && artifact.sha256 !== qc.sha256) {
      throw new Error("CODEX_ARTIFACT_HASH_MISMATCH");
    }
    const finalPath = resolve(this.config.runtimeDir, "artifacts", `${job.id}.png`);
