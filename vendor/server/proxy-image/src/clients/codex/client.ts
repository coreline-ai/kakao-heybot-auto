...
export class CodexClient {
  ...
  constructor(private readonly config: ImageProxyConfig) {
    this.#secret = readSecret(config.codexSecretFile);
  }
  ...
  async readiness(signal?: AbortSignal): Promise<{ ready: boolean; reason?: string }> {
    try {
      ...
      return { ready: false, reason: "CODEX_UNAVAILABLE" };
    }
  }

  async create(requestId: string, prompt: string, signal: AbortSignal): Promise<CodexJobResponse> {
    const response = await fetch(`${this.config.codexBaseUrl}/internal/v1/codex/jobs`, {
      method: "POST",
      signal,
      ...
      body: JSON.stringify({
        ...
        artifactContract: {
          ...
          maxArtifacts: 1,
          maxBytesPerArtifact: this.config.imageMaxBytes,
        },
      }),
    });
  ...
  async get(jobId: string, signal: AbortSignal): Promise<CodexJobResponse> {
    const response = await fetch(
      `${this.config.codexBaseUrl}/internal/v1/codex/jobs/${encodeURIComponent(jobId)}`,
  ...
  async download(
    ...
    const response = await fetch(
      `${this.config.codexBaseUrl}/internal/v1/codex/jobs/${encodeURIComponent(jobId)}/artifacts/${encodeURIComponent(artifactId)}`,
      { signal, headers: this.headers() },
    );
    if (!response.ok || response.headers.get("content-type") !== "image/png") {
      throw new Error("CODEX_ARTIFACT_DOWNLOAD_FAILED");
    }
    const declared = Number(response.headers.get("content-length") || 0);
    if (declared > this.config.imageMaxBytes) throw new Error("IMAGE_SIZE_INVALID");
    const data = Buffer.from(await response.arrayBuffer());
    if (data.length > this.config.imageMaxBytes) throw new Error("IMAGE_SIZE_INVALID");
    return data;
  }
  ...
  private headers(contentType?: string): Record<string, string> {
    return {
      ...
      "x-heybot-service-id": this.config.codexServiceId,
