...
- `펜브러쉬` 불허용: 새 펜브러쉬 생성·상태·취소·재전송 요청을 중단하고,
  이미 완료된 결과도 전송 직전에 capability revision을 다시 확인한다.
- `이미지분석` 불허용: 새 분석 요청을 중단하고 완료 결과도 전송 직전에 버린다.
...
이미지 생성과 이미지 분석은 서로 다른 방 권한이다. 분석이 허용된 방에서 이미지를
먼저 전송한 뒤 다음 명령을 입력한다.
...
- 답장이 아니면 같은 방에서 참여자가 설정 시간 안에 보낸 최신 분석 가능 이미지 1장을 선택한다. 명령 사용자와 이미지 발신자가 달라도 되며, 방 밖의 이미지는 선택하지 않는다. 최근 범위의 운영 기본값은 30분이며 `IRIS_VISION_RECENT_IMAGE_WINDOW_MS`로 변경할 수 있다.
...
- 앱이 재시작되어 메모리가 비어 있어도 정확한 답장과 같은 방의 최근 이미지는 `chat_logs` DB에서 다시 찾는다.
- 헤이봇이 생성·전송한 이미지를 포함해 같은 방의 최근 분석 가능 이미지는 `헤이봇 이미지 분석`으로 선택할 수 있다. 특정 이미지를 확실히 지정하려면 그 이미지에 직접 답장해 명령한다. 이미지 전송만으로 자동 분석하지는 않는다.
- 실행 권한은 `IRIS_BOT_ROOM_POLICY_FILE`의 `이미지분석` capability만 사용한다. `IRIS_VISION_ALLOWED_CHAT_IDS` 같은 별도 정적 allowlist는 사용하지 않는다. 정책에 없는 방은 fail-closed하며 자동 등록·자동 허용되지 않는다.
- `헤이봇 이미지분석 허용 R01` → 미리보기 → `헤이봇 방 적용 <코드>` 순서로 켠다.
- `헤이봇 이미지분석 불허용 R01`도 같은 확인 절차를 사용한다.
- 원본은 exact `talk.kakaocdn.net` HTTPS 주소만 허용하며 redirect, 만료 URL, 10 MiB 초과,
  MIME·magic 불일치를 거부한다. 서명 URL과 이미지 원본은 응답·로그에 표시하지 않는다.
