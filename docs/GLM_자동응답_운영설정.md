...
- 조건: 허용된 3개 오픈방의 실제 `chat_id`와 `헤이봇` 호출어가 모두 일치
...
- 수신 메시지는 `헤이봇` 호출어가 있을 때만 외부 Z.AI API에 전달된다. 단, 코어라인 AI 연구소 관리자가 전역 일반대화 모드를 켠 동안 허용방의 일반 텍스트는 `REPLY`·`WAIT`·`IGNORE` 판정용으로만 전달될 수 있다.
...
| `IRIS_GLM_ALLOWED_CHAT_IDS` | `18480337854645134,18226456888539938,18243496625741211` | 코어라인 AI 연구소 및 주식 오픈채팅방 2곳 |
...
스크립트는 `/data/local/private`가 `700 root:root`, 토큰·기존 기억·기존 관리자 파일이
`600 root:root`인지 내용 노출 없이 검사한다. 관리자 파일이 없으면 경고 후 기동하며 관리자
명령만 비활성화된다.
...
| `헤이봇 대화 시작` | 코어라인 AI 연구소의 관리자 | 모든 GLM 허용방의 호출어 없는 일반대화 판정 시작 |
| `헤이봇 대화 상태` | 코어라인 AI 연구소의 관리자 | 전역 일반대화 on/off와 적용 방 수 확인 |
| `헤이봇 대화 종료` | 코어라인 AI 연구소의 관리자 | 모든 허용방의 호출어 없는 일반대화 즉시 중단 |
...
| `GlmFailure.Timeout` | 120초 안에 응답 없음 | 네트워크·Z.AI 상태 확인, 대체 모델 또는 다음 호출에서 재시도 |
